...
class LocationStayRuleSuite extends FunSuite with BeforeAndAfter {
	...
	test("2 测试已有用户的MC的处理(cache中有缓存信息)") {
		...
		val mc4 = MCSourceObject(1, sdf.parse("20150401 08:58:00.000").getTime, 111, 1, 13900000001L, 13910000001L)
		...
		assert(mc4.getLabel(Constant.LABEL_STAY).get("area1").get == 20 * 60 * 1000 + "")	//没有通过
	}
	...
	test("11 测试某用户在多个区域多个停留时间设置的处理") {
		...
		assert(mc8.getLabel(Constant.LABEL_STAY).get("area3").get == 10 * 60 * 1000 + "")	//没有通过
		...
		assert(mc9.getLabel(Constant.LABEL_STAY).get("area2").get == 20 * 60 * 1000 + "")	//没有通过
