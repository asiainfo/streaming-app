 ...
 */
class Load2SingleHashThread(lines: Array[String], columnSeperator: String,
                        hashName: String,
                        fieldIdxes: Array[Int], fieldSeperator: String, conversion10to16Idxes: Array[Int],
                        valueIdx: Int, valueMapEnabled: Boolean, valueMap: String,
                        jedisPool: JedisPool,
                        loadMethod: String, batchLimit: Int, 
                        overwrite: Boolean, appendSeperator: String) extends Runnable {
  ...
  override def run(): Unit = {
    ...
    for(line <- lines){
      ...
      logger.debug("numProcessed = "+numProcessed+", numBatches = "+numBatches +", numInBatch = "+numInBatch +", line=>"+lineArray.mkString("[",",","]"))

      val field = (for(idx <- fieldIdxes)
        yield if (conversion10to16Idxes.contains(idx)) RedisUtils.convertDecimaltoHex(lineArray(idx)) else lineArray(idx)
