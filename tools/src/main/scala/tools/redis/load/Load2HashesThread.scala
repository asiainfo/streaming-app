...
class Load2HashesThread(lines: Array[String], columnSeperator: String,
  ...
  override def run(): Unit = {
    ...
    logger.debug("[debug0] linesSize = " + lines.length)
    for(line <- lines){
      logger.debug("[debug1] numProcessed = "+numProcessed+",numBatches = "+ numBatches +", numInBatch = "+ numInBatch +", line => " + line.split(columnSeperator).mkString("[",",","]"))
