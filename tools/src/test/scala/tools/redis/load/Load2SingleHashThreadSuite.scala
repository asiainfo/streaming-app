...
class Load2SingleHashThreadSuite extends FunSuite with BeforeAndAfter {
  ...
  test("1 测试100条记录，1个线程，每6条记录批量提交， hset，覆盖加载"){
    ...
    //数据 2,3,value1-test-2,value2-test-3
  ...
  test("2 测试100条记录，1个线程，每6条记录批量提交， hset"){
  ...
  test("3 测试100条记录，1个线程，每6条记录批量提交， hset"){
  ...
  test("4 测试100条记录，1个线程，每6条记录批量提交， hset，追加加载"){
    ...
    val worker2 = new Load2SingleHashThread(lines2.toArray, columnSeperator,
      hashName, fieldIdxes, fieldSeperator,
      valueIdxes, valueSeperator,
      jedisPools(0),loadMethod, batchLimitForRedis, overwrite, appendSeperator)
    ...
    assert(rs1.get("2:3").split(appendSeperator).toSet == Set("value1-test-2#value2-test-3","value1-test2-2#value2-test2-3"))
    assert(rs1.get("8:9").split(appendSeperator).toSet == Set("value1-test-8#value2-test-9","value1-test2-8#value2-test2-9"))
    assert(rs1.size() == 100)
  }
