...
class Kafka2hdfsSuite extends FunSuite with BeforeAndAfter {
  ...
  test("test createHadoopFileIfNonExist") {
    ...
    Kafka2Hdfs.createHadoopFileIfNonExist(fs, dir1, true)
    Kafka2Hdfs.createHadoopFileIfNonExist(fs, file1, false)
    ...
    Kafka2Hdfs.createHadoopFileIfNonExist(fs, hadoop_outputWithPrefix, false)
